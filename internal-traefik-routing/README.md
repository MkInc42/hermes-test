...
Nothing in this project changes or recreates an application container. `intake-api` is reached by its existing Docker DNS name over the external `proxy-public` network. Phish and filedrop are transitional `host.docker.internal` routes using Docker's `host-gateway` mapping. The Hermes backend remains defined for a possible future route, but no router currently exposes it.

## Routes

| Internal hostname | Destination | Notes |
|---|---|---|
| `phish.192-168-1-115.sslip.io` | UI `:8088`; API paths to `:8012` | `/v1*`, `/health`, `/docs*`, `/redoc*`, and `/openapi.json` are sent to the API for same-origin UI use. |
| `phish-api.192-168-1-115.sslip.io` | native Phish API `:8012` | All paths go to the API. |
| `kanban.192-168-1-115.sslip.io` | Reserved; disabled | No router is configured. The native Hermes route redirects to `/login`, which returns 404 under this hostname, and Hermes has no separate proxy authentication. Keep disabled pending a working login flow and a real auth boundary. |
| `filedrop.192-168-1-115.sslip.io` | trip-filedrop `:8787` | Transitional; uploaded content is sensitive. |
| `intake.192-168-1-115.sslip.io` | `intake-api:8000` | Directly over `proxy-public`; does not use host port 8017. |
| `traefik.192-168-1-115.sslip.io/dashboard/` | `api@internal` | Dashboard/API route protected by the same IP allowlist; `api.insecure` is disabled. |
...
`sslip.io` is a public wildcard DNS service even though these names resolve to a private address. DNS queries may leave the LAN; use local DNS/hosts entries instead if hostname query privacy matters.
...
Prefer the Docker provider when the app is containerized. Attach the app container to the existing external `proxy-public` network and add labels like these, replacing `myapp`, the hostname, and port with app-specific values:
...
services:
  myapp:
    ...
    labels:
      ...
      - "traefik.http.routers.myapp.rule=Host(`myapp.192-168-1-115.sslip.io`)"
